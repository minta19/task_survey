...
from .serializers import (SignUpSerializer,CreateSurveySerializer,SurveyListSerializer,SurveyDetailSerializer,QuestionAddSerializer,
                          MyUnpublishedSurveyDetailSerializer,SurveyPublishSerializer,ResponseForSurveySerializer,SurveyEditSerializer,QuesEditSerializer)
...
class MyUnpublishedSurvey(generics.ListAPIView):
     ...
     def get_queryset(self):
          ...
          return Survey.objects.filter(created_by=user,is_published=False)
...
class SurveyTitleEdit(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if survey_to_update.created_by != user:
            return Response({"message": "You are not allowed to edit this survey."}, status=status.HTTP_403_FORBIDDEN)

        if survey_to_update.is_published:
            return Response({"message": "You are not allowed to edit a published survey."}, status=status.HTTP_403_FORBIDDEN)
        ...
        return Response({"message": "Survey title updated successfully."}, status=status.HTTP_200_OK)
...
class SurveyQuestionEdit(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if question_to_update.survey.created_by != user:
            return Response({"message": "You are not allowed to edit this question."}, status=status.HTTP_403_FORBIDDEN)
...
class SurveyQuestionCreate(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            survey = Survey.objects.get(id=survey_id, created_by=user, is_published=False)
        except Survey.DoesNotExist:
            return Response({"message": "Survey not found or it's published."}, status=status.HTTP_404_NOT_FOUND)
...
class SurveyResponseView(generics.CreateAPIView):
    ...
    def create(self, request, survey_id, *args, **kwargs):
        ...
        if SurveyResponse.objects.filter(survey=survey, user=user).exists():
            return Response({'detail': 'You have already attempted this survey.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for response_data in responses:
            ...
            if question.survey != survey or question.question_number != question_number:
                return Response({'detail': 'Invalid question ID or number.'}, status=status.HTTP_400_BAD_REQUEST)

            if question.question_type == 'objective':
                options = question.options
                if answer not in options:
                    raise ValidationError({'detail': 'Invalid answer for the objective question.'})
        ...
        if len(valid_responses) == len(responses):
            SurveyResponse.objects.bulk_create([SurveyResponse(**data) for data in valid_responses])  
        try:
            score = Score.objects.get(user=user, survey=survey)
        except Score.DoesNotExist:
            score = Score.objects.create(user=user, survey=survey, total_score=total_score)
