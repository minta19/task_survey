    ...
    def validate_questions(self, questions):
        ...
        if len(questions) > max_questions:
            raise ValidationError(
                f"Maximum allowted number of questions are {max_questions}"
            )
        if len(questions) < min_questions:
            raise ValidationError(
                f"Minimum allowted number of options are {min_questions}"
            )
            ...
            if question_type == "objective":
                if options:
                    if len(options) > max_options:
                        raise ValidationError(
                            f"Maximum allowted number of options are {max_options}"
                        )
                    if len(options) < min_options:
                        raise ValidationError(
                            f"Minimum allowted number of options are {min_options}"
                        )
        return questions
    ...
    def create(self, validated_data):
        ...
        for question_data in questions_data:
            Question.objects.create(
                survey=survey, question_number=question_number, **question_data
            )
            question_number += 1
        return survey
...
class SurveyListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Survey
        fields = ['id', 'title', 'created_by', 'created_time', 'is_published']
